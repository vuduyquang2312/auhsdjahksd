...
export default function Booking() {
    ...
    const handleSuggestionClick = (suggestion) => {
        // Cập nhật giá trị cho input đang active
        if (activeInput === 'pickup') {
    ...
    const fetchSuggestions = async (query) => {
        if (query.length >= 2) {
            ...
            const processedSuggestions = data
                .filter(item => item.display_name.includes("Vietnam") && item.display_name.includes("Hà Nội"))
                .map(item => {
                    let displayName = item.display_name;
                    displayName = displayName.replace(/Province/g, "");
                    displayName = displayName.replace(/District/g, "");
